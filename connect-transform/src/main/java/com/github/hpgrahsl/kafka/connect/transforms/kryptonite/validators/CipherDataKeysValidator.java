...
public class CipherDataKeysValidator implements Validator {
  ...
  static {
    VALID_KEY_LENGTHS.addAll(List.of(16,32,64));
  }
  ...
  public void ensureValid(String name, Object o) {
    try {
      ...
      if(!dataKeyConfig.stream()
          ...
          .filter(dkc -> dkc.getMaterial().getKey().stream()
              .anyMatch(kc -> !kc.getKeyData().getValue().isEmpty()))
          .map(DataKeyConfig::getKeyBytes)
          .allMatch(bytes -> VALID_KEY_LENGTHS.contains(bytes.length - TinkKeyConfig.RAW_KEY_BYTES_OFFSET))) {
  ...
  public String toString() {
    return "JSON array holding at least one valid data key config object, e.g. [{\"identifier\":\"my-key-123-abc\",\"material\": <tink_key_config_json_here>}]";
  }
