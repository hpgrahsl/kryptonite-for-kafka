...
public abstract class JsonStringReader<R extends ConnectRecord<R>> implements Transformation<R> {

  public static final ConfigDef EMPTY_CONFIG_DEF = new ConfigDef();

  private static final ObjectMapper OBJECT_MAPPER = new ObjectMapper();
  private static final Logger LOG = LoggerFactory.getLogger(JsonStringReader.class);
  ...
  public R apply(R r) {
    ...
    if (!(data instanceof String)) {
      ...
      throw new DataException("error: data expected to be of type String but was "
          + data.getClass());
    }

    try {
      var jsonMap = OBJECT_MAPPER.readValue((String) data, LinkedHashMap.class);
      return newRecord(r, jsonMap);
  ...
  public static class Key<R extends ConnectRecord<R>> extends JsonStringReader<R> {
  ...
  public static class Value<R extends ConnectRecord<R>> extends JsonStringReader<R> {
