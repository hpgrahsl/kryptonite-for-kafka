...
### 👉 For the latest module documentation please go to the official docs page for the [Keyset Tool](https://hpgrahsl.github.io/kryptonite-for-kafka/dev/keyset-tool/) 👈
...
A command-line tool for generating Tink keyset configurations (JSON or YAML) used by all _Kryptonite for Kafka_ modules. Supports both plaintext and KMS-encrypted keyset generation using GCP Cloud KMS, AWS KMS, or Azure Key Vault.
...
The runnable JAR is created at `target/kryptonite-keyset-tool-0.2.0-SNAPSHOT.jar`.

## Usage

```bash
java -jar target/kryptonite-keyset-tool-0.2.0-SNAPSHOT.jar [OPTIONS]
```
...
| `-k, --num-keysets`   | No               | `1`     | Number of keysets to generate (1-1000). When > 1, output is an array (JSON) or sequence (YAML) with identifiers suffixed `_1`, `_2`, etc.                                                           |
| `--initial-key-id`    | No               | `10000` | Starting key ID, incremented by 1 for each additional key                                                                                                                                            |
| `-o, --output`        | No               | stdout  | Output file path                                                                                                                                                                                     |
| `-p, --pretty`        | No               | `false` | Pretty-print JSON output (single-line by default; ignored for YAML)                                                                                                                                 |
...
### Single AES-GCM keyset (FULL format, JSON pretty-printed)

```bash
java -jar target/kryptonite-keyset-tool-0.2.0-SNAPSHOT.jar \
...
```json
{"primaryKeyId":10000,"key":[{"keyData":{"typeUrl":"type.googleapis.com/google.crypto.tink.AesGcmKey","value":"<BASE64_ENCODED_KEY_HERE>","keyMaterialType":"SYMMETRIC"},"status":"ENABLED","keyId":10000,"outputPrefixType":"TINK"}]}
```
...
### Deterministic encryption keyset (AES-GCM-SIV, JSON)
...
### Deterministic encryption keyset (AES-GCM-SIV, YAML)

```bash
java -jar target/kryptonite-keyset-tool-0.2.0-SNAPSHOT.jar \
  -a AES_GCM_SIV -i my-det-key -f FULL --yaml
```
...
### Multiple keysets as YAML sequence

```bash
java -jar target/kryptonite-keyset-tool-0.2.0-SNAPSHOT.jar \
  -a AES_GCM -i demo-key -f FULL -k 3 -n 2 --initial-key-id 10000 --yaml
```

Output: a YAML sequence with 3 keysets, each containing 2 keys:
...
```bash
java -jar target/kryptonite-keyset-tool-0.2.0-SNAPSHOT.jar \
  -a AES_GCM -i my-key -f FULL --yaml -o /path/to/keyset.yaml
```
