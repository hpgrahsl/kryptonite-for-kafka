...
public class EncryptMapWithSchemaUdf extends AbstractCipherFieldWithSchemaUdf {
    ...
    public @Nullable Map<?, String> eval(
    ...
    private Map<?, String> process(final Object data, final String schemaString,
        ...
        if (!schema.startsWith("MAP<")) {
            throw new IllegalArgumentException(
                    "when encrypting maps schema string must represent a MAP<...> type - got: " + schemaString);
        }
        ...
        var mapType = (MapType) dataType.getLogicalType();
        ...
        for (Map.Entry<?, ?> entry : ((Map<?, ?>) data).entrySet()) {
            result.put(entry.getKey(), encryptData(entry.getValue(), valueType, fmd));
        }
        return result;
    }
