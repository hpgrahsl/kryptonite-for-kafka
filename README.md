...
After that, start using the UDFs, namely `K4KENCRYPT` and `K4KDECRYPT`, to selectively encrypt / decrypt column values in ksqlDB rows of `TABLES` and `STREAMS` respectively. **Read here about the configuration options and how to [apply the UDFs](ksqldb-udfs-kryptonite/README.md) based on simple examples.**
...
The project uses authenticated encryption with associated data ([AEAD](https://en.wikipedia.org/wiki/Authenticated_encryption)) and in particular applies [AES](https://en.wikipedia.org/wiki/Advanced_Encryption_Standard) in [GCM](https://en.wikipedia.org/wiki/Galois/Counter_Mode) mode for probabilistic encryption (default) or [SIV](https://en.wikipedia.org/wiki/AES-GCM-SIV) mode for uses cases which either require or at least benefit from deterministic encryption. The preferred and new default way is to configure Kryptonite to use Google's [Tink](https://github.com/tink-crypto) multi-language, cross-platform open-source cryptography library.

#### Kryptonite for Kafka version 0.4.0+

The cipher algorithms below are provided out of the box. Others could be added by extending the core library with your own implementations:
...
#### Kryptonite for Kafka version 0.6.0+

Kryptonite for Kafka additionally provides **[Format Preserving Encryption](https://en.wikipedia.org/wiki/Format-preserving_encryption) (FPE)** using the FF3-1 algorithm. Unlike standard AEAD encryption which produces variable-length ciphertext, FPE maintains the original format and length of the plaintext data.

_NOTE: Preferably, the library should rather provide FF1 which is a different FPE cipher algorithm. However, there are patent claims and copyrights which forbid its usage in open-source software. As there aren't any other significantly better FPE cipher alternatives left, FF3-1 was chosen as the next best thing that is NIST vetted and has at least seen some reasonable adoption in the field._
