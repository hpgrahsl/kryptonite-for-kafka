...
class JsonSchemaDeriverTest {
    ...
    @Nested
    @DisplayName("computeStableHash")
    class ComputeStableHash {
        ...
        void deterministicForSameInputs() {
            ...
            String h1 = JsonSchemaDeriver.computeStableHash(42, Set.of(fc));
            String h2 = JsonSchemaDeriver.computeStableHash(42, Set.of(fc));
            assertThat(h1).isEqualTo(h2);
        }

        @Test
        @DisplayName("hash is exactly 8 hex characters")
        void hashIsEightHexChars() {
            var fc = FieldConfig.builder().name("age").build();
            String hash = JsonSchemaDeriver.computeStableHash(42, Set.of(fc));
            assertThat(hash).hasSize(8).matches("[0-9a-f]{8}");
        }

        @Test
        @DisplayName("different encryptedSchemaId produces different hash")
        void differentSchemaIdDifferentHash() {
            var fc = FieldConfig.builder().name("age").build();
            String h1 = JsonSchemaDeriver.computeStableHash(42, Set.of(fc));
            String h2 = JsonSchemaDeriver.computeStableHash(99, Set.of(fc));
            assertThat(h1).isNotEqualTo(h2);
        }

        @Test
        @DisplayName("different field names produce different hash")
        void differentFieldNamesDifferentHash() {
            ...
            String h1 = JsonSchemaDeriver.computeStableHash(42, Set.of(fcAge));
            String h2 = JsonSchemaDeriver.computeStableHash(42, Set.of(fcName));
            assertThat(h1).isNotEqualTo(h2);
        }

        @Test
        @DisplayName("field ordering does not affect hash (sorted internally)")
        void fieldOrderingIndependent() {
            var fcAge = FieldConfig.builder().name("age").build();
            var fcName = FieldConfig.builder().name("name").build();
            String h1 = JsonSchemaDeriver.computeStableHash(42, Set.of(fcAge, fcName));
            String h2 = JsonSchemaDeriver.computeStableHash(42, Set.of(fcName, fcAge));
            assertThat(h1).isEqualTo(h2);
        }
    }
}
